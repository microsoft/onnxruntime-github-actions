...
async function run__ninja(terrapinTool, version, hash) {
  const platformFile = (() => {
    const platform = getPlatformIdentifier();
    if (platform === 'linux') {
      if (process.arch === 'arm64') return 'ninja-linux-arm64.zip';
      if (process.arch === 'x64') return 'ninja-linux.zip';
    } else if (platform === 'macos') {
      return 'ninja-mac.zip';
    } else if (platform === 'windows') {
      if (process.arch === 'arm64') return 'ninja-winarm64.zip'; // sic, no `-` between `win` and `arm64`
      if (process.arch === 'x64') return 'ninja-win.zip';
    }

    throw new Error(`Unsupported architecture for Ninja download on ${platform} ${process.arch}`);
  })();
  ...
  // for now they're the same, but let's keep it separate for consistency with CMake.
  // impl, but leave undocumented from the readme. don't want people depending on this unless it becomes necessary.
