...
const mockStream = {
    ...
    _triggerError: (err) => { // Helper to trigger error
...
describe('Common Utils', () => {
    ...
    describe('executeCommand', () => {
        it('should execute command successfully and return stdout/stderr', async () => {
            ...
            expect(exec.getExecOutput).toHaveBeenCalledWith('my-cmd', ['arg1', 'arg2'], expect.objectContaining({ ignoreReturnCode: true }));
        ...
        it('should mask GITHUB_TOKEN when sensitive option is true', async () => {
            ...
            expect(exec.getExecOutput).toHaveBeenCalledWith('login', ['-p', 'my-secret-token'], expect.objectContaining({ sensitive: true, ignoreReturnCode: true }));
        ...
        it('should throw error and log details on non-zero exit code', async () => {
            ...
            expect(exec.getExecOutput).toHaveBeenCalledWith('fail-cmd', ['--bad'], expect.objectContaining({ ignoreReturnCode: true }));
         ...
         it('should not mask token if sensitive is false', async () => {
            ...
            expect(exec.getExecOutput).toHaveBeenCalledWith('login', ['-p', 'my-secret-token'], expect.objectContaining({ sensitive: false, ignoreReturnCode: true }));
            delete process.env.GITHUB_TOKEN;
        });
    });
    ...
    describe('hashFileContent', () => {
        ...
        it('should throw other errors encountered during stat', async () => {
            ...
            expect(fsPromises.stat).toHaveBeenCalledWith(testFilePath);
            expect(core.error).toHaveBeenCalledWith(`Error accessing file ${testFilePath} for hashing: ${otherError.message}`);
        });
    });
    ...
    describe('parseBuildArgs', () => {
        ...
        it('should parse simple key-value pairs', () => {
            expect(utils.parseBuildArgs('KEY1=VAL1 KEY2=VAL2')).toEqual([
                '--build-arg', 'KEY1=VAL1', '--build-arg', 'KEY2=VAL2'
            ]);
        });

        it('should parse pairs with quotes', () => {
            expect(utils.parseBuildArgs('A=1 B=\'Hello World\' C="Another Arg"')).toEqual([
                '--build-arg', 'A=1', '--build-arg', "B='Hello World'", '--build-arg', 'C="Another Arg"'
            ]);
        });
        ...
        it('should handle args without spaces', () => {
         ...
         it('should handle complex values within quotes', () => {
            expect(utils.parseBuildArgs('A=1 KEY="Value with = sign"')).toEqual([
                '--build-arg', 'A=1', '--build-arg', 'KEY="Value with = sign"'
            ]);
        });
    });
    ...
    describe('checkPathExists', () => {
        ...
        it('should return false and warn if fs.stat rejects with other error', async () => {
            ...
            expect(result).toBe(false);
            expect(fsPromises.stat).toHaveBeenCalledWith(testPath);
            expect(core.warning).toHaveBeenCalledWith(`Path check: Error checking '${testPath}': ${otherError.message}. Assuming it does not exist.`);
        });
    });
