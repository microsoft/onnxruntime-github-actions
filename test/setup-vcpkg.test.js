...
const { run, getInputs, downloadVcpkgZip, extractVcpkgZip, findActualVcpkgDir, bootstrapVcpkg, cacheVcpkgDir, cleanupDownload } = setupVcpkg; // Destructure functions

// --- Test Suite ---
describe('Setup vcpkg Action - Helper Functions', () => { // Updated describe name
    ...
    beforeEach(() => {
        ...
        core.getInput.mockImplementation(name => {
            switch (name) {
                case 'vcpkg-version': return TEST_VERSION;
                case 'vcpkg-hash': return TEST_HASH;
                case 'terrapin-tool-path': return TEST_TERRAPIN_PATH;
                default: return '';
            }
        });
        core.group.mockImplementation(async (name, fn) => await fn());
        core.setFailed.mockImplementation(msg => { throw new Error(`MOCK_setFailed: ${msg}`); });
        ...
        tc.downloadTool.mockImplementation(async (url, dest) => {
            ...
            ensureDirExistsSync(finalDest); vol.writeFileSync(finalDest, 'zip content'); return finalDest;
        });
        tc.extractZip.mockImplementation(async (zipPath, dest) => {
            ...
            ensureDirExistsSync(vcpkgDir); vol.mkdirSync(vcpkgDir);
    ...
    const setPlatform = (platform) => { Object.defineProperty(process, 'platform', { value: platform, writable: true }); };
    ...
    describe('extractVcpkgZip', () => {
     ...
     describe('findActualVcpkgDir', () => {
        ...
        it('should return nested path if only one directory exists and primary does not', () => {
            ...
            ensureDirExistsSync(expectedPath); vol.mkdirSync(expectedPath);
            ensureDirExistsSync(path.join(TEST_TEMP_EXTRACT_PATH, 'some_file.txt')); vol.writeFileSync(path.join(TEST_TEMP_EXTRACT_PATH, 'some_file.txt'), 'data');

            const result = findActualVcpkgDir(TEST_TEMP_EXTRACT_PATH, TEST_VERSION); // Use imported helper

            expect(result).toBe(expectedPath);
            expect(core.warning).toHaveBeenCalledWith(expect.stringContaining(`Expected folder pattern 'vcpkg-${TEST_VERSION}' not found`));
            expect(core.warning).toHaveBeenCalledWith(expect.stringContaining(`Assuming single nested directory '${nestedDirName}'`));
        });
        ...
        it('should return path without v prefix if version has v and that path exists (and not single dir)', () => {
            ...
            ensureDirExistsSync(pathWithoutV); vol.mkdirSync(pathWithoutV);
            ensureDirExistsSync(otherDirPath); vol.mkdirSync(otherDirPath);
            ...
            expect(vol.readdirSync(TEST_TEMP_EXTRACT_PATH)).toEqual(expect.arrayContaining([`vcpkg-${TEST_VERSION}`, 'another_dir'])); // Check dir contents were read
            expect(vol.existsSync(pathWithoutV)).toBe(true); // Check fallback was checked (and implicitly succeeded)
            expect(warningSpy).toHaveBeenCalledWith(expect.stringContaining(`Expected folder pattern 'vcpkg-${versionWithV}' not found`));
            expect(warningSpy).toHaveBeenCalledWith(expect.stringContaining(`Found extracted dir matching tag without 'v' prefix`));
        ...
        it('should throw if primary path missing and multiple directories exist', () => {
            ...
            ensureDirExistsSync(dir1); vol.mkdirSync(dir1);
            ensureDirExistsSync(dir2); vol.mkdirSync(dir2);
        ...
        it('should throw if primary path missing and only files exist', () => {
            ...
            ensureDirExistsSync(file1); vol.writeFileSync(file1, 'content');
            ensureDirExistsSync(file2); vol.writeFileSync(file2, 'content');
        ...
        it('should throw if primary path missing and v-prefix fallback also missing', () => {
             ...
             ensureDirExistsSync(otherFilePath1); vol.writeFileSync(otherFilePath1, 'data');
             ensureDirExistsSync(otherFilePath2); vol.writeFileSync(otherFilePath2, 'data');
             ...
             expect(warningSpy).toHaveBeenCalledWith(expect.stringContaining(`Expected folder pattern 'vcpkg-${versionWithV}' not found`));
             expect(warningSpy).not.toHaveBeenCalledWith(expect.stringContaining(`Found extracted dir matching tag without 'v' prefix`));
             expect(warningSpy).not.toHaveBeenCalledWith(expect.stringContaining('Assuming single nested directory'));
    ...
    describe('bootstrapVcpkg', () => {
         it('should run .bat on Windows', async () => {
             ...
             ensureDirExistsSync(scriptPath); vol.writeFileSync(scriptPath, 'echo Bootstrapping...');
         ...
         it('should chmod and run .sh on Linux', async () => {
              ...
              ensureDirExistsSync(scriptPath); vol.writeFileSync(scriptPath, '#!/bin/sh');
              await bootstrapVcpkg(TEST_EXTRACTED_VCPKG_DIR_EXACT); // Use imported helper
              expect(vol.existsSync(scriptPath)).toBe(true); // Verify test setup
         ...
         it('should throw if script not found', async () => {
              setPlatform('win32');
              ensureDirExistsSync(TEST_EXTRACTED_VCPKG_DIR_EXACT); vol.mkdirSync(TEST_EXTRACTED_VCPKG_DIR_EXACT); // Dir exists, script doesn't
    ...
    describe('cleanupDownload', () => {
         it('should delete file if it exists', () => { // Renamed for clarity
             ensureDirExistsSync(TEST_DOWNLOAD_ZIP_PATH);
